...
def main():
    ...
    for sr in srs:
        ...
        np.savetxt("mr_explicit_regions/reals_{o}_{s}_{im}.txt".format(
            o=order, s=sr, im=imex), reals)
        np.savetxt("mr_explicit_regions/imags_{o}_{s}_{im}.txt".format(
            o=order, s=sr, im=imex), imags)
        np.savetxt("mr_explicit_regions/rs_{o}_{s}_{im}.txt".format(
            o=order, s=sr, im=imex), rs_max)
        plt.plot(reals, imags, label="SR={}".format(sr))

    plt.title("Multi-Rate Adams Explicit Stability Search, Order {order}, mu = {mu}"
              .format(order=order, mu=-r_mu))
    plt.legend()
    # plt.show()
    plt.savefig("mr_explicit_regions/mr_{order}_{sr}_{im}.png".format(
