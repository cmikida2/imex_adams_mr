...
def ab_trig(h, order, time_hist, t):
    ...
    for i in range(0, order):
        ...
        if (i+1) == 1:
            ...
            coeff_rhs[i] = -1/(n*alpha)*np.cos(n*alpha*h)
        else:
            coeff_rhs[i] = 1/(n*alpha)*np.sin(n*alpha*h)
        for j in range(0, order):
            m = (j+1) // 2
            if (j+1) % 2 == 0:
                vdmt[i][j] = np.sin(alpha*m*thist[i])
            else:
                vdmt[i][j] = np.cos(alpha*m*thist[i])
...
def am_trig(h, order, time_hist, t):

    # TRIG BASES
    alpha = 0.7
    thist = time_hist.copy() - t + 1
    am = np.zeros(order)
    vdmt = np.zeros((order, order))
    coeff_rhs = np.zeros(order)
    for i in range(0, order):
    ...
    am = np.linalg.solve(np.transpose(vdmt), coeff_rhs)
...
def main():
        ...
        for i, re in enumerate(reals):
            for j, im in enumerate(imags):
                ...
                if order == 1:
                    ...
                    time_hist[2] = -2
                    # First explicit microstep
                    ab = ab_trig((1/sr), order, time_hist, 0)
                    ynp_n = 1 + ab[0]*lbda
                    ynp_nm1 = ab[1]*lbda
                    ynp_nm2 = ab[2]*lbda
                    # First IMEX macrostep to get trial state
                    # this will be used in state interp for substep args
                    ab_mac = ab_trig(1, order, time_hist, 0)
                    if am_ext:
                        am_mac = am_trig(1, order, time_hist, 0)
                    ...
                    for k in range(2, sr+1):
                        # get AB coeffs.
                        ab = ab_trig((1/sr), order, time_hist, (k-1)/sr)
    ...
    plt.title("Trig IMEX Adams MR Order {order} Stability Regions, "
