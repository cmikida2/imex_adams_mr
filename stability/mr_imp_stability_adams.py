...
def main():
    ...
    t_end = 100
    dt = 1
    # This code should never be run with orders 1 or 2 with lbda = 0
    # these are A-stable and the code will run forever
    order = 2
    srs = [1, 2, 3, 4, 5]
    ...
    for sr in srs:
        ...
        for r_lbda in rs_lbda:
            ...
            for ith, theta in enumerate(thetas):
                ...
                print("Theta = ", theta)
                # Find stability by expanding outward
                # (we've had previous problems with bisection)
                r = 0
                dr = 10
        ...
        # np.savetxt("mr_implicit_regions/reals_{o}_{s}_{im}.txt".format(
        #     o=order, s=sr, im=imex), reals)
        # np.savetxt("mr_implicit_regions/imags_{o}_{s}_{im}.txt".format(
        #     o=order, s=sr, im=imex), imags)
        # np.savetxt("mr_implicit_regions/rs_{o}_{s}_{im}.txt".format(
        #     o=order, s=sr, im=imex), rs_max)
        plt.plot(reals, imags, label="SR={}".format(sr))

    plt.title(
            "Multi-Rate Adams Implicit Stability Search, Order {order}, lbda = {lbd}"
            .format(order=order, lbd=-r_lbda))
    plt.legend()
    # plt.show()
    plt.savefig("mr_implicit_regions/mr_{order}_{sr}_{im}.png".format(
        order=order, sr=sr, im=imex))
